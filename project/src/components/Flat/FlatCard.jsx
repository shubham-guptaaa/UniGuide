...
const FlatCard = ({ flat }) => {
  return (
    <div className="overflow-hidden bg-white rounded-lg border border-gray-200 shadow-md transition-transform duration-300 hover:shadow-lg hover:-translate-y-1">
      ...
      <div className="p-6">
        <h2 className="mb-2 text-xl font-semibold text-gray-800">{flat.flat_name}</h2>
        ...
        <div className="mb-4">
          ...
          <p className="text-gray-600">
            <span className="font-medium">₹{flat.rent_per_month.toLocaleString()}</span>
          </p>
        </div>
        ...
        <div className="flex justify-between items-center pt-4 mt-4 border-t border-gray-200">
          <p className="text-gray-600">{flat.contact_info}</p>
          <a 
            href={flat.source_link} 
            target="_blank" 
...
const FlatCardList = ({ flatData }) => {
  const { isCollapsed } = useSidebar();
  const [searchTerm, setSearchTerm] = useState('');
  const [priceRange, setPriceRange] = useState({ min: '', max: '' });
  const [showPriceFilter, setShowPriceFilter] = useState(false);
  
  const filteredFlats = flatData.filter(flat => {
    const searchLower = searchTerm.toLowerCase();
    const locationString = `${flat.location.area} ${flat.location.city}`.toLowerCase();
    const matchesSearch = flat.flat_name.toLowerCase().includes(searchLower) || 
                         locationString.includes(searchLower);
    
    const matchesPrice = (!priceRange.min || flat.rent_per_month >= Number(priceRange.min)) &&
                        (!priceRange.max || flat.rent_per_month <= Number(priceRange.max));
  ...
  return (
    <div className={`px-4 py-8 ${isCollapsed ? 'ml-20' : 'ml-64'} transition-all duration-300`}>
      <div className="flex relative gap-2 justify-end mb-6">
        <input
          type="text"
          placeholder="Search by flat name or location..."
          value={searchTerm}
          ...
          <div className="absolute right-0 top-12 z-10 p-4 bg-white rounded-lg border border-gray-200 shadow-lg">
            <div className="space-y-3">
              <div className="flex flex-col">
                <label className="mb-1 text-sm text-gray-600">Min Rent (₹)</label>
                <input
                  type="number"
              ...
              <div className="flex flex-col">
                <label className="mb-1 text-sm text-gray-600">Max Rent (₹)</label>
                <input
                  type="number"
